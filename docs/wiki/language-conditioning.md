...
**"grounding까지만 언어가 관여하고, 액션은 순수 기하학"이라는 분해형(Decomposed) 파이프라인이 현재 우리 리소스(150ep 규모)에서 안전하게 도달 가능한 최대치라는 게 CH38의 판정이다.** π0/TinyVLA/SmolVLA/RoboVLMs는 모두 언어가 액션 생성까지 직접 관여하지만, 우리는 텍스트→bbox(Stage1 PaliGemma2 LoRA, 분리도 gap +95%p 실증)까지만 끊어서 기하 제약을 강제하고, Stage2 MLP는 cx/area 같은 순수 좌표만 받는다.

**"객체가 뭔지"는 신경망이 강하게 반영하지만 "어느 쪽으로 가라"는 방향/스타일 지시는 표현(hidden state) 레벨에서도, 출력 레벨에서도 거의 도달하지 않는다.** CH38-4/38-5/CH42: basket vs ball처럼 객체가 바뀌면 cos_dist 0.36~0.44로 강한 신호가 나오지만, left/right 같은 방향 지시는 cos_dist 0.03~0.05로 10배 약하고, 실제 생성 출력에서는 방향 단어가 bbox에 전혀 반영되지 않는다(4세션 전부 노이즈 수준). frozen probe로도 prompt 종류 자체는 99%로 구분되지만 이건 "다른 토큰이라 다르다"일 뿐 방향 이해가 아니며, 진짜 이미지 방향 신호는 거짓 prompt를 줘도 90%대로 전혀 안 흔들린다.

**CH61에서 과거 "언어 조건화 성공"으로 보였던 실험(43ep 프록시, counterfactual changed_rate 20.3%)이 버그 수정 후(vis_feat L2-정규화 누락 수정) 정확히 0.0%로 붕괴했다** — 즉 텍스트를 참조는 하나(permutation −16.1%p) "명령"이 아니라 "경로 맥락(prior)"으로만 쓰인다. 근본 원인은 데이터 자체: 조이스틱 수집 구조상 지시문과 실제 경로가 1:1로 상관돼 있어(text-scene confound) 언어를 따르는 신호 자체가 데이터에 없다 — 코드/학습 레벨 우회(정규화 수정, bbox_scale, PG2 비전 교체, synthetic augmentation)를 모두 시도했지만 전부 무효했고, 재수집만이 유일한 해법으로 재확인됐다(61-12~61-16).

**CH68은 과거 "language-conditioned" 실험(train_step2_instr_head.py, permutation drop 14.6%p)이 실제로는 라벨 누출이었음을 밝혔다** — 지시문이 path_type에서 합성돼 "curve to the left"가 정답 클래스를 자연어로 직접 지시했기 때문. 진짜 증명에는 "같은 초기 프레임 · 다른 지시문 → 다른 타겟"인 다중 객체 과제가 필요하며(비전만으로 원리적으로 풀 수 없는 구조), 액션 어휘("left/right/turn") 금지·no_text/shuffled_text/wrong_text 3종 대조군·방향 일치율을 주 지표로 사전 고정하는 프로토콜(L0~L4 단계)이 세워졌다.

**L1(지시문→검출 프롬프트 교체)은 실측 완료, 성립함**: 같은 이미지에서 "gray basket"→cx 0.484, "chair"→cx 0.089로 조향 부호가 55.1% 반전 — 이는 학습이나 코드 수정 없이 이미 동작하는 "language-directed target selection"이다. 하지만 **액션까지 바뀌는가(L1을 액션 레벨로 확장)는 실측 결과 실패했다(68-8)**: bbox를 chair로 교체해도 방향 일치율 35.1%로 우연(50%)보다도 낮았다. 원인 규명(68-9, cx만 통제 변수로 스윕) 결과, 배포 헤드는 cx 0.05~0.95 전 구간에서 좌질량이 음수(우측 선호가 역전 없이 유지)로 나와, **cx→조향 방향의 입력-출력 대응 자체가 헤드 안에서 붕괴돼 있다**는 게 밝혀졌다. 흥미롭게도 미러 증강(65-9) 헤드는 이 cx 응답 대응관계를 정상 방향(Spearman −0.575)으로 복구시켰는데, 이는 실기 100건에서 아직 검증되지 않은 결과다.

...
**총 규모**: 트랙 A 180개 + 트랙 B 60개 = **240개**. 예산이 빠듯하면 트랙 A만 먼저
끝내고(도달 성능 개선 확인), 트랙 B는 여유 될 때 추가하는 순서 권장.
...
## 2. 시간 배분 (트랙A 180개 + 트랙B 60개 = 240개 기준)
...
| 실패/재촬영 버퍼 (20%) | 1.7~2.4시간 | 조이스틱 미숙, 센서 이슈 |
| vis feature 인코딩 (신규 240ep) | 20~35분 | 오늘 150ep 인코딩 시간 비례 추정 |
...
- [ ] **(SODA, 2026-07-10) 트랙A가 거리(depth)를 전혀 통제하지 않음**: 지금 대시보드
      트랙A 진행률은 cx(좌우 위치)만 4구간으로 나눠 추적하고, 로봇-바구니 사이 실제
      물리적 거리는 통제/기록이 안 됨 — 같은 "강한좌"라도 오퍼레이터가 가깝게 놓든
      멀게 놓든 다 같은 칸으로 카운트됨. 위에 있는 "실제 물리 공간에서 몇 미터인지"
      항목과 직결된 문제. 옵션: ① 위치×경로×거리(근/원)로 더 세분화(4×3×2=24칸,
      개당 목표 줄이거나 전체 규모 확대) ② 지금처럼 오퍼레이터 재량에 맡기고 안 건드림
      ③ 실로봇 캘리브레이션 실측치(W 1회≈12~13cm, 2026-06-26 실측) 활용해 거리도 명시 태깅.
      결정 필요.

#!/usr/bin/env python3
"""
CH43-2b의 LSTM 결과(none 95.87%, add 96.85%, replace 95.47%)가 closed-loop
SR(실주행 성공률)로도 이어지는지 확인 — CH40에서 "PM은 올라도 SR은 안
오른다"고 본 문제가 LSTM에서도 재현되는지가 핵심 질문(사용자 요청).

CH37 결론(override 없는 게 최선)에 맞춰 override 없이 argmax 그대로
trajectory를 만들어 비교. val 29~30개 에피소드, baseline은 동일 seed=42 split.

Usage:
  .venv/bin/python3 scripts/eval/closed_loop_eval_lstm.py
"""
import sys
import json
...
def main():
    ...
    out_path = ROOT / "docs/v5/closed_loop_eval/lstm_comparison.json"

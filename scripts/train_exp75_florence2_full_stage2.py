#!/usr/bin/env python3
"""exp75 — Florence-2 완전 통합 Stage 2 (그라운더+비전인코더+프로젝션 전부 교체, 2026-08-19).

배경 (사용자 요청, "세개 다 바꿔서 하도록 진행해줘"):
  지금까지는 요인 교란을 피하려고 하나씩만 바꿔서 검증했다:
    exp74            — 비전 인코더만 Florence-2 (그라운더=OWL-v2 그대로)
    그라운더 스왑     — 그라운더만 Florence-2 (비전 인코더=Kosmos-2 그대로)
  이 스크립트는 **둘을 합쳐서** 그라운더(bbox)와 비전 인코더(vis) 둘 다
  Florence-2로 교체한 완전 통합 버전을 만든다. image_proj(투영 레이어)는 이미
  Stage1에서 Florence-2 특징에 맞춰 재학습된 것(stage1_florence2_5cls_projs.pt)을
  그대로 쓴다 — 즉 "그라운더+비전인코더+프로젝션" 셋 다 Florence-2 계열.

  텍스트 프로젝션/텍스트 인코더는 여전히 Kosmos-2 그대로다 — 배포 시 호출되지
  않는 컴포넌트이고, 지금까지 어떤 실험에서도 안 건드렸다(요인 통제).

바꾸는 것 (exp74와의 차이는 bbox 출처 하나뿐):
  베이스라인(exp73)    : bbox=OWL-v2        · vis=Kosmos-2
  exp74               : bbox=OWL-v2        · vis=Florence-2
  그라운더 스왑        : bbox=Florence-2    · vis=Kosmos-2
  exp75(이 스크립트)   : bbox=Florence-2    · vis=Florence-2   ← 완전 통합

재사용(재인코딩 없음, 캐시 전부 기존 자산):
  1024d 원시 특징      : docs/v5/detector/stage1_florence2_feats.npz (Stage1에서 생성)
  bbox 주석            : docs/v5/bbox_nav_florence2/bbox_dataset_v6_florence2.json (그라운더 스왑에서 생성)
...
비교 기준:
  ...
  그라운더 스왑(bbox만)     val_acc mean 73.26%±0.29%p · best 73.59% (L/ROT_L 회귀)

주의: 실기 검증은 별건이다. 확정 발견 6번 — val 지표와 실기 성능은 직결되지 않음.

출력:
  docs/v5/closed_loop_eval/exp75_florence2_full_vis_cache.pt
  runs/v5_nav/mlp/exp75_florence2_full/exp75_florence2_full_v6_mlp.pt
...
ANN_FLORENCE = ROOT / "docs/v5/bbox_nav_florence2/bbox_dataset_v6_florence2.json"
FEAT_NPZ    = ROOT / "docs/v5/detector/stage1_florence2_feats.npz"
CACHE_OUT   = ROOT / "docs/v5/closed_loop_eval/exp75_florence2_full_vis_cache.pt"
OUT_DIR     = ROOT / "runs/v5_nav/mlp/exp75_florence2_full"
REPORT      = ROOT / "docs/v5/closed_loop_eval/exp75_florence2_full_stage2.json"
...
def main():
    ...
    for seed in SEEDS:
        ...
        print(f"  [exp75/mlp] seed={seed} val_acc={acc*100:.2f}%", flush=True)

    mean, std = float(np.mean(accs)), float(np.std(accs))

    ckpt = OUT_DIR / "exp75_florence2_full_v6_mlp.pt"
    torch.save({"model": best_state, "val_acc": best_overall, "head": "mlp",
                "window": WINDOW, "bbox_scale": BBOX_SCALE, "exp": "exp75",
                "backbone": FLOR, "grounder": "florence2", "stage1": str(STAGE1_F2)}, str(ckpt))

    rep = {
        "exp": "exp75", "backbone": FLOR, "grounder": "florence2 (DENSE_REGION_CAPTION 우선 + OD 폴백)",
    ...
    print("\n" + "=" * 70)
    print("  exp75 — Florence-2 완전 통합 (그라운더+비전인코더+프로젝션)")

# 초안 빨간글씨 채우기 — 붙여넣기용 초고

> 대상: `EdgeGround-VLA_바퀴형로봇_목표지향주행_경량모델_초안_0805.docx`
> 작성 2026-08-05 · 액션플랜 1번(본문 사실오류 정정) + 2번(22건 초고)
> 근거 페이지: [빨간글씨 25건 문제/대응](https://minuum.github.io/MoNaVLA/v5/paper_redfill_worklist.html)
...
`[삽입]` — 빨간글씨 메모를 산문으로 풀어 쓴 것입니다. 메모 자체는 지워도 됩니다.
...
> 곱한다.
>
> 행동 헤드는 최근 6프레임(`window=6`)의 프레임 특징을 이어붙인 1560차원 벡터를 입력받아
> 8개 행동 클래스의 로짓을 출력한다. 시간 문맥을 명시적으로 포함시킨 것은 그라운딩이 매
> 프레임 수행되지 않고 일정 주기로만 갱신되기 때문이며(1.4절), 헤드가 직전 관측의 변화를
> 통해 정지 상태와 진행 상태를 구분할 수 있게 한다.
>
...
빨간글씨 04번은 `(area는 bbox_scale=3.0 배율 적용)`이라고 되어 있으나, 실제로는
...
> 검출기의 신뢰도 임계값은 운영 환경에서 0.20으로 설정하였다. 이 값은 사전 실험에서 결정한
> 0.25를 실기 조건에 맞춰 완화한 것이다. 임계값 스윕 결과 0.25에서 정탐률 95.3%를 유지하며
...
> 검출에 실패하면 위의 주기를 무시하고 즉시 재검출을 수행한다. 여기에 회전 동작과 관련된 세
> 가지 보호 규칙을 함께 적용한다. 첫째, 회전 동작을 수행한 직후에는 시야가 변했으므로 직전
> 검출 결과를 재사용하지 않고 반드시 재검출한다. 둘째, 한 번의 회전 지속 시간을 0.4초로
> 제한하여 과회전으로 목표가 시야를 벗어나는 것을 막는다. 셋째, 회전을 연속으로 선택하지
> 못하게 하여 회전과 재검출이 반복되는 상태에 갇히는 것을 방지한다.
>
...
> 정지 판정은 기하 규칙이 아니라 학습된 정지 클래스(class 0)의 예측으로 수행한다. 다만 에피소드
...
>
> 수집 규모는 225 에피소드, 16,599 프레임이다. 시작 위치를 좌측 계열과 우측 계열로 나누어
> 각각 90 에피소드씩 대칭적으로 배치하였고, 그라운딩 라벨의 화면상 수평 위치 평균은
> 0.4976으로 좌우 균형을 확인하였다.
>
> 다만 에피소드 수의 대칭이 행동 클래스의 대칭을 보장하지 않았다. 좌측 계열 행동은 3,384
> 프레임, 우측 계열 행동은 4,122 프레임으로 21.8%의 차이가 있었으며, 특히 대각 직진에서
> 좌측 2,532 대 우측 3,117의 차이가 나타났다. 동일한 시작 위치 배치에서도 좌회전 경로의
> 프레임당 회전량이 달라 누적 프레임 수가 어긋난 결과이다. 이 불균형은 후술하는 좌우 성능
> 비대칭의 출발점이 된다.
>
...
`[유지]` 기존 빨간글씨 목록을 그대로 표로 옮기고, **아래 3항목을 추가**합니다.
...
> 성능이 과대평가된다.
>
> 학습 샘플은 매 프레임이 아니라 5프레임 간격의 결정 시점마다 하나씩 구성하고, 해당 구간의
> 정답 행동 중 다수를 라벨로 사용하였다. 이는 운영 시 그라운딩이 일정 주기로만 갱신되어 같은
> 판단이 여러 프레임 유지되는 구조(1.4절)를 학습에도 반영한 것이다.
...
> 학습된 행동 헤드 체크포인트와 선형 사상 가중치를 로봇에 전달하고, 로봇 내부에서 추론 서버를
> 실행한다. 서버는 카메라 프레임을 입력받아 행동 클래스를 반환하는 형태로 동작한다.
>
...
> 선택한다.
>
> 후처리 단계에서 1.4절의 제어 규칙을 적용한다. 시작 직후 정지 억제, 회전 지속 시간 제한,
> 연속 회전 차단, 검출 실패 시 강제 재검출을 순서대로 검사한 뒤 최종 행동을 로봇 속도 명령으로
> 변환한다.
>
...
기존 빨간글씨 문단이 **이미 정확합니다.** 아래 한 문단만 보태면 서술이 완결됩니다.

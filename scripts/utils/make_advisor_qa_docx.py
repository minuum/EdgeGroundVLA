...
H('격차 방어 — 프레임 단위 정확도 ≠ 에피소드 단위 성공률', 11)
P('행동 헤드의 학습·검증 정확도 격차(91.44% 대 74.04%)에는 과적합의 영향이 포함되어 있으나, 이 격차를 '
  '그대로 성능 저하로 해석하는 것은 적절하지 않다. 두 지표의 측정 단위가 서로 다르기 때문이다. 검증 '
  '정확도는 매 장면(프레임)에서 사람이 조작한 단 하나의 행동과 일치하는지를 묻는 반면, 실기 성공률은 '
  '에피소드 단위로 목표에 도달했는지를 묻는다. 특정 시점에서 목표에 접근할 수 있는 행동은 일반적으로 '
  '하나가 아니므로(목표가 약간 왼쪽에 있을 때 FORWARD와 FWD+LEFT는 모두 접근에 유효하다), 전자는 '
  '후자보다 훨씬 엄격한 기준이다.', color=AINEW, indent=0.5)
d.add_paragraph()
P('이 주장은 검증 혼동행렬로 확인된다. 오류 629건을 심각도로 분해하면 다음과 같다.',
  color=AINEW, indent=0.5)
TBL([
 ['오류 성격','건수','전체 대비','오류 대비'],
 ['좌우 반전 (치명적)','48','1.97%','7.6%'],
 ['같은 방향 성향 내 혼동 (대체 가능한 행동)','512','21.06%','81.4%'],
 ['STOP 판정 시점 차이','69','2.84%','11.0%'],
])
FIG2='docs/v5/figures/action_head_error_severity.png'
if os.path.exists(FIG2):
    d.add_picture(FIG2, width=Cm(16.5))
    P('좌: 8×8 혼동행렬을 셀 성격별로 구분(초록=정답, 노랑=같은 방향 성향 내 혼동, 빨강=좌우 반전, '
      '회색=STOP 판정 차이). 우: 좌·우·중립 3방향 축약 시 81.65%.', color=GREY, sz=8, indent=0.5)
    d.add_paragraph()
P('즉 오류의 81.4%는 틀린 행동이라기보다 같은 방향으로 접근하는 다른 유효한 선택이며, 주행을 실제로 '
  '실패시키는 좌우 반전은 전체 판단의 1.97%에 불과하다. 특히 FORWARD와 FWD+LEFT/FWD+RIGHT 사이의 '
  '상호 혼동이 오류의 52.5%(330건)를 차지하는데, 이 네 경우를 정답으로 간주하면 정확도는 87.70%가 되고, '
  '행동을 좌·우·중립 세 방향으로 축약하면 81.65%가 된다.', color=AINEW, indent=0.5)
d.add_paragraph()
P('또한 에피소드 단위 지표가 더 관대할 수 있는 구조적 이유가 있다. 폐루프 제어에서 로봇은 매 판단 '
  '시점마다 갱신된 관측으로 다시 판단하므로, 한 시점에서 대체 행동을 선택하더라도 다음 시점에 목표 '
  '위치가 재측정되어 스스로 보정된다. 에피소드당 평균 11.64회의 판단이 이루어지는 구조에서 개별 판단의 '
  '불일치는 누적되지 않고 흡수된다. 반면 프레임 단위 채점은 이러한 재정정 기회를 반영하지 못하고 매 '
  '프레임을 독립적으로 감점한다. 따라서 검증 정확도는 실기 성능의 하한(lower bound)에 가깝게 해석하는 '
  '것이 타당하며, 검증 74.1%와 실기 95%는 상충하는 결과가 아니다.', color=AINEW, indent=0.5)
d.add_paragraph()

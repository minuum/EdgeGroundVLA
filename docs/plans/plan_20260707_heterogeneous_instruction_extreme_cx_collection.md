...
**추가 결정 필요**: 트랙A/B에 "일부러 과도하게 꺾은 뒤 되돌아오는(오버슈트→재수렴) 궤적"을
세팅당 몇 회 명시적으로 포함할지 — 현재 트랙A/B 모두 이 유형이 없음. 초반 1회 회전
에피소드를 데이터셋에서 빼는 방안은 **비권장**(패턴 자체가 문제가 아니라 다양성 부족이
문제라 삭제보다 추가가 맞음). 상세: `docs/v5/closed_loop_eval/CH62_FORWARD_LOCK_AND_LABEL_CONFOUND.md`

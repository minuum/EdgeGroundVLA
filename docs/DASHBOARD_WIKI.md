...
## 경량화(라즈베리파이) 실측 — 파라미터·레이턴시·양자화 (2026-07-29)

논문 방향이 "경량화로 라즈베리파이 탑재 가능성 입증"으로 전환됨에 따라 실측한
파라미터/리소스/양자화 결과. **파라미터 수만으로 "가볍다"고 판단하면 안 됨** —
아래 표처럼 파라미터가 5배 작은 쪽(OWL-v2)이 오히려 레이턴시는 32배 더 걸림.

| 구성요소 | 파라미터 | 로딩시간 | GPU 추론 지연(fp32) | GPU 추론 지연(fp16) |
|---|---|---|---|---|
| Kosmos-2 vision_model만 | 0.303B (전체 로드 1.664B) | 35.0s | **58.6ms** | — |
| OWL-v2 (base-patch16-ensemble) | 0.155B | 3.9s | **1901.7ms** | **962.1ms** |

- **fp16 전환**: OWL-v2에 `torch_dtype=torch.float16` 적용 시 지연시간 **2배 개선
  (1901.7ms→962.1ms)**, GPU 메모리도 절반(1.98GB→1.00GB). 정확도 손실은 별도
  실기 검증 필요하지만, 코드 한 줄로 바로 적용 가능한 손쉬운 개선.
- **GPU 없는 조건(CPU만, 라즈베리파이 유사)**: OWL-v2 fp32 CPU 추론은
  **21,512ms(21.5초)/프레임** — 이 상태로는 실사용 불가능한 수준.
- **int8 동적 양자화**: 이 Jetson의 PyTorch 빌드는
  `torch.backends.quantized.supported_engines == ['none']` — **양자화 백엔드
  (qnnpack/fbgemm) 자체가 컴파일 안 되어 있어 시도조차 불가능**. 라즈베리파이
  경량화를 실제로 추진하려면 (a) qnnpack 지원 PyTorch 빌드로 교체하거나
  (b) ONNX Runtime/TensorRT 등 별도 추론엔진으로 변환하는 작업이 선행되어야 함
  — "코드 몇 줄" 수준이 아니라 별도 검토 과제.

> [!info]
> 결론: 경량화 방향을 검토할 때 **파라미터 수보다 실측 레이턴시 프로파일을
> 기준으로 판단**하는 게 안전함. 지금 조건에서 OWL-v2가 병목이고, fp16은 바로
> 적용 가능한 개선이지만 int8은 인프라 교체가 선행돼야 함.

...
STAGE1_PT  = ROOT / "runs/v5_nav/mlp/stage1_v3_5cls/stage1_v3_5cls_owl_projs.pt"  # 신규(5-class,225ep,OWL)
ANN_V6     = ROOT / "docs/v5/bbox_frame_level/bbox_dataset_v6_pg448_cx.json"   # --ann-v6로 교체 가능 (owl ablation)
CACHE_V6   = ROOT / "docs/v5/closed_loop_eval/exp73_v6_vis_cache_stage1v3.pt"  # 새 image_proj로 재인코딩 — 별도 캐시(기존 파일 보존)
CACHE_V5   = ROOT / "docs/v5/closed_loop_eval/exp71_vis_cache_colorfixed.pt"
OUT_DIR    = ROOT / "runs/v5_nav/mlp/exp73_stage1v3"
OUT_LOG    = ROOT / "docs/v5/closed_loop_eval/exp73_stage1v3_trackA_heads.json"
...
class FiLMHead(nn.Module):
    ...
    concat(가산적 결합) 대신 cx가 "vis를 어떻게 해석할지"를 조절하는 곱셈적 결합."""
...
def train_cxaux(X_tr, y_tr, X_va, y_va, seed, epochs=300, lr=5e-4, aux_weight=0.3):
    """CxAuxHead 전용 학습 — main CE + aux_weight*MSE(cx_pred, 현재 프레임 cx)."""
    torch.manual_seed(seed)
    np.random.seed(seed)
    ...
    for ep in range(epochs):
        ...
        for i in range(0, len(perm), 128):
            ...
            cx_true = xb[:, -1, 0]
            loss = F.cross_entropy(logit, yb, weight=weights_t) + aux_weight * F.mse_loss(cx_pred, cx_true)
            opt.zero_grad(); loss.backward(); opt.step()
        sched.step()
        if ep % 25 == 0 or ep == epochs - 1:
            model.eval()
            with torch.no_grad():
                logit, _ = model(X_va_t)
                acc = (logit.argmax(1) == y_va_t).float().mean().item()
            if acc >= best_acc:
                best_acc = acc
                best_state = {k: v.clone() for k, v in model.state_dict().items()}
    model.load_state_dict(best_state)
        ...
        for head in args.heads.split(","):
            ...
            for seed in [int(s) for s in args.seeds.split(",")]:
                if head == "contreg":
                    ...
                    acc, state, per_class = train_cxaux(
                        X_tr, y_tr, X_va, y_va, seed, epochs=args.epochs)
                elif head == "chunk":

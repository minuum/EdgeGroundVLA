...
이러한 한계는 PaliGemma의 SigLIP 비전 인코더만 튜닝하고 DINOv2 레이어를 고정한 비전 튜닝의 불균형, 그리고 소규모 데이터(224ep) 환경에서 LM 레이어까지 튜닝하여 발생한 특정 텍스트 프레이즈 과적합(Overfitting)에 기인합니다. 

이에 따라 비전 인코더 전체(SigLIP + DINOv2 상위 레이어)로 LoRA 튜닝 범위를 넓히고 LM 레이어를 제외하는 아키텍처 재설계를 수행하되, **LoRA 재학습 후에도 OOD 객체(그레이 컨테이너 등) 인식 성능이 개선되지 않을 경우**에 대비해야 합니다. 사전학습 지식이 풍부한 **기본 프리트레인드 객체(Pre-trained Object Classes)로 교체하여 데이터를 재수집하는 백업 플랜**을 다음과 같이 제안합니다.

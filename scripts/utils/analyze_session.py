...
def analyze_session_log(json_path: str):
    ...
    print("=" * 60)
    print(f"📊 세션 분석 리포트: {data.get('session_id', 'UNKNOWN')}")
    print(f"🗓️ 시작 시간: {data.get('timestamp', 'UNKNOWN')}")
    print(f"🤖 사용 모델: {data.get('model_name', 'UNKNOWN')}")
    print(f"💬 주행 지시어: {data.get('instruction', 'UNKNOWN')}")
    print(f"📡 추론 모드: {data.get('instruction_mode', 'UNKNOWN')}")
    print("=" * 60)
    ...
    if len(latencies) == 0:
        print("⚠️ 스텝 데이터가 부족하여 분석을 수행할 수 없습니다. (초기화 스텝만 기록됨)")
        print("-" * 60)
        return
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        for arg in sys.argv[1:]:
            analyze_session_log(arg)
    else:
        # Default: scan and analyze all session JSON logs in docs/inference_reports
        script_dir = Path(__file__).resolve().parent
        reports_dir = script_dir.parent.parent / "docs" / "inference_reports"
        session_files = sorted(reports_dir.glob("session_*.json"))
        if session_files:
            print(f"🔍 Found {len(session_files)} session logs in {reports_dir}. Analyzing all...")
            for s_file in session_files:
                analyze_session_log(str(s_file))
        else:
            # Fallback to the local path of the latest session
            target_session = "/home/minum/26CS/MoNaVLA/docs/inference_reports/session_20260604_085923.json"

...
3. **vis 캐시 재생성 + 학습** (`train_exp73_trackA_heads.py`)
   ...
   - **[2026-07-19 변경] 3-seed 전체를 개별 체크포인트로 저장할 것** — best-of-3만
     저장하는 기존 방식은 63-14에서 seed 간 closed-loop 분산(33.3~60.6%)이 크다는
     게 드러나 더 이상 충분하지 않음. 289ep 재학습 시 seed 0/1/2 각각 저장 →
     3개 전부 closed-loop 평가 → 평균±표준편차로 보고.
...
## 6. [2026-07-19 추가] 3개 실험(action-chunk / temporal smoothing / seed 분산) 결과

사용자 지시("3→1→2 순서로") 순서대로 진행:

1. **action-chunk 헤드(K=4, temporal ensembling)** — 새 `ActionChunkHead` 구현.
   앙상블 자체는 가설대로 유효(+3pp: 39.4%→42.4%)하나 헤드 base 성능이 mlp보다
   낮아(offline 77.1% vs 85.3%) 종합 순위는 못 넘음. CH63 63-12.
2. **mlp에 causal temporal smoothing 후처리** — 정반대 결과. window 3/5/7/9 전부
   단조 악화(60.6%→18.2%). 원인: 청크 앙상블은 "같은 시점"의 여러 독립 예측을
   평균한 것이지만, causal smoothing은 "다른 시점"의 예측을 평균해 방향 전환 시
   반응 지연만 유발. **mlp는 스무딩 없이 그대로 배포가 맞음.** CH63 63-13.
3. **mlp 3-seed 개별 closed-loop 검증** — 61-11에서 미뤘던 항목. **중대 발견**:
   seed 0/1/2 재학습 결과 offline val_acc는 76.9~78.3%로 거의 동일한데 closed-loop
   Success는 33.3%~48.5%로 크게 갈림 — 게다가 "동일 seed"로 재학습해도 원래
   "best"(60.6%) 체크포인트가 재현되지 않음(GPU 비결정성). **60.6%는 "진짜 1위"가
   아니라 이 아키텍처가 낼 수 있는 성능 분포의 낙관적 상단으로 봐야 함.** val 33ep
   표본 크기 + GPU 비결정성이 만든 노이즈가 63-11 리더보드의 순위 신뢰도 자체를
   약화시킴 — mlp(33~60%)·hybrid(39.4%)·chunk(42.4%) 구간이 실제로 겹칠 가능성.
   CH63 63-14.

**결론**: 트랙C(64ep) 도착 후 289ep 재학습부터는 **단일 체크포인트 1회 평가가 아니라
최소 3-seed 평균/분산으로 보고**해야 함 — §D 재학습 런북에 반영 필요(아래 D 항목
갱신). 표본이 225→289ep로 늘면 분산이 다소 줄어들 것으로 기대되나 근본적으로는
val 세트 크기 자체를 키우는 것(트랙C 전량 활용 등)도 고려해볼 만함.

...
> 근거 페이지: [논문 드래프트 보완 25건 문제/대응](https://minuum.github.io/MoNaVLA/v5/paper_redfill_worklist.html)
...
> 출력을 병치(concatenate)한 것으로, 앞의 4채널은 오픈 보캐블러리 검출기 OWL-v2가 산출한
> 기하 정보(`cx`, `cy`, `area`, `has_bbox`)이고 뒤의 256채널은 Kosmos-2 비전 타워의
...
>
> 전체 파이프라인에서 학습되는 구성요소는 행동 헤드(0.866M 파라미터)뿐이다. OWL-v2(0.155B),
> Kosmos-2 비전 타워(0.303B), 선형 사상(0.262M)은 모두 동결(frozen)되며 별도의 특화
> 파인튜닝을 수행하지 않는다. 즉 본 연구의 학습 비용은 전체 파라미터가 아니라 0.866M에
> 국한되고, 이것이 제안 구조가 온디바이스 환경에서 반복 실험을 감당할 수 있는 근거이다.
...
> **수정 — 서빙 제어는 이 도표에서 분리했다.** 처음에는 서빙 제어 파라미터를 이 모델
...
> 융합이 아니라, **서로 다른 성질의 두 표현을 병치하여 행동 헤드가 각각을 직접 참조하도록
...
> 임계값 스윕 결과 0.25에서 정탐률 95.3%를 유지하며
...
> 1.4절(서빙 제어)과 1절(모델 구조)에 산문으로 풀어 두었다. 이 표는 그 값들을 빠르게
...
>
> 그럼에도 행동 클래스 분포는 대칭이 아니었다. 전체에서 좌측 계열 행동은 3,384 프레임,
> 우측 계열은 4,122 프레임으로 21.8% 차이가 있다. 이 차이를 두 묶음으로 분해하면 원인이
> 좁혀진다.
...
> 불균형은 **트랙 A에서만 발생하고 트랙 F에서는 사실상 없다**. 또한 정렬 회복에 필요한
> 조향 자체는 좌우가 대칭이었다 — 강좌 시작 에피소드의 우측 계열 비율 19.2%와 강우 시작
> 에피소드의 좌측 계열 비율 19.9%는 0.7%p 차이이고, 약좌·약우 쌍도 1.6%p 차이다.
>
> 실제로 다른 것은 **직진의 비중**이다. 좌측 시작 에피소드는 직진이 49~51%인데 우측 시작
> 에피소드는 40~41%로 약 10%p 낮고, 그만큼 조향 행동이 더 많이 사용되었다. 이 초과분이
> 우측 계열 총량을 키웠다. 따라서 이 불균형은 어느 한 방향의 조향을 회피한 결과가 아니라,
> **시작 위치에 따라 조작자가 사용한 조향의 총량이 달랐던 결과**로 해석된다.
>
> 이 구분이 중요한 이유는 대응 방법이 달라지기 때문이다. 에피소드 수나 프레임 수는 이미
> 대칭이므로 좌측 데이터를 더 모으는 것으로는 해결되지 않는다. 실제로는 클래스 가중치와
> 좌우 미러 증강으로 대응하였다(5.2절, V절). 이 불균형은 후술하는 좌우 성능 비대칭의
> 출발점이 된다.
>
...
> 값 자체가 아니라, 모든 실험 arm(원본 데이터·미러 증강·hold-aware 등)이 **동일한 시드로 생성된
...
배포 헤드(hold-aware, OWL-v2 grounder, 3 seed)를 val 분할(에피소드 단위, 결정 시점
501개)에 적용한 결과다.

![val 혼동행렬](../v5/figures/confusion_matrix.png)

val 정확도는 74.98%±0.52%p(3 seed)이다. 클래스별로 보면 STOP(92%)과 ROT_L(100%)의
정확도가 높은 반면, **LEFT는 42%로 가장 낮고 그 중 33%가 FWD+L로 오분류**된다. RIGHT는
65%로 상대적으로 높다. 이 비대칭은 우연이 아니라 좌우 비대칭 원인 분석(CH66)에서 확인한
것과 같은 현상이다 — 학습 데이터의 좌/우 조향 총량 차이가 헤드의 좌측 방향 판단력을
약화시켰고, 그 결과가 이 혼동행렬의 LEFT 행에 직접 나타난다.

### 실기(Closed-loop) 결과

100회 실기 테스트(5개 시작 위치 × 20회)의 목표 도달 성공률은 89/100(89.0%)이다. 위치별
분해에서 중앙 100%·약우 95%·강우 90%·강좌 80%·약좌 80%로, **좌측 시작에서 가장 낮다** —
위 혼동행렬의 LEFT 클래스 취약성과 방향이 일치한다(IV절).

> 서술 시 유의: 혼동행렬(프레임 단위 판단 정확도)과 실기 성공률(에피소드 단위 도달 여부)은
> 척도가 다른 지표다. 중간에 몇 프레임을 잘못 판단해도 재검출·서빙 제어로 보정되면 에피소드
...
상단의 **서빙 제어 파라미터 요약**(검출 임계값, 그라운딩 호출 간격, 콜드스타트 가드, 회전
...
> 검출 임계값을 포함한 서빙 제어 파라미터는 추론 서버 실행 시 환경변수로 주입되며, 지정하지
...
> 걸쳐 있고, 이것이 서빙 제어 규칙에 의존하게 된 구조적 이유이다.
...
| 정지 판정 | "학습된 정지 클래스로 수행" | 코드 **기본값은 근접 판정**(area ≥ 0.25, \|cx−0.5\| ≤ 0.35, 3연속). 학습 클래스 방식은 환경변수로 선택 | `STOP_MODE` 기본 `proximity` |
...
- **운영 임계값 0.20** — 위 27건에는 `owlv2_thresh = 0.25`로 기록되어 있습니다(이 세션들은
  PG2-448 그라운더 시기). 0.20은 실기 담당자 보고와 CH64 64-17 기준이며,
  **100건 테스트 세션의 원본 H5가 현재 로컬에 없어 직접 확인하지 못했습니다.**
  → 논문에 쓰기 전에 해당 세션의 `runtime_config`를 재확인해야 합니다.
- **`stop_mode = learned`** — 위 27건의 `runtime_config`에 이 키가 없어 값을 확인할 수 없습니다.

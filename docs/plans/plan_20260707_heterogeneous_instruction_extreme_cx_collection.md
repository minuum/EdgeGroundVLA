...
- [ ] 트랙C(오버슈트→재수렴) 시나리오 키 추가 — CH62 근거, 신규
      ...
      - **화면 레이아웃 배치(2026-07-12 정정)**: 대시보드 상단 "🕹️ 조작(키보드 W/A/S/D/Q/E/Z/C/R/T)"
        블록과 **같은 행**에 이미 2:1:1 컬럼 비율 구조가 있음 — 1열(2fr)=키보드 조작 블록
        유지, **중간 열(가운데 1fr)에 "📼 에피소드 제어"(episode_name/시작/정지&저장/복귀)
        블록과 그 아래 "📊 현재 에피소드 타임라인"(기호 시퀀스+실측 Hz) 블록을 함께 배치**
        (기존에 다른 위치에 있던 이 두 블록을 이 중간 열로 이동). 3열(마지막 1fr)은 기존
        조이스틱(DragonRise) 블록 그대로 유지. ※ 이전 버전에서 "오버슈트 방향 순환축 UI"를
        이 자리에 넣으라고 잘못 적었던 것을 정정 — 오버슈트 방향은 기존 D-pad 좌/우 순환
        축(위 항목 참고)으로만 처리하고 별도 위젯은 불필요.

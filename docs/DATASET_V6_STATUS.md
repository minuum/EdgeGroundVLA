...
## 🟢 [2026-09-22] pos2_약우 재수집 완료(300건 확정) + 그라운딩 인코더 3종 실측 비교 + 실패 케이스 유형 정리

**pos2_약우 재수집 완료** — 0/20(archived) → 20/20(100%)로 채워져서 pos1+pos2+pos3
300건 세트가 최종 확정됐습니다. 통합 성공률: pos1 95.0% / pos2 91.0% / pos3
90.0% / 전체 92.0%.

**그라운딩 인코더 3종 신규 실측 비교 (2026-09-20)** — pos1/2/3 실제 세션에서
성공/실패 섞어 뽑은 이미지 12장을 OWLv2·PG2·Kosmos-2 세 그라운더에 전부
실제로 통과시켰습니다(OOM 방지 위해 서버 내리고 PG2→Kosmos-2 순차 단독
로드, 재기동 후 config diff로 완전 동일 확인). 결과:
- OWLv2 8/12 정합 검출, PG2 1/12(나머지 대부분 full-frame 붕괴 — 옛 로그
  현상이 최신 이미지로도 재현됨), Kosmos-2 12/12 `has_bbox=True`지만 raw
  output에 "basket" 단어가 단 한 번도 안 나옴(전부 벽/TV/책상/의자 등 오탐) —
  실질 정답은 0/12
- 속도: Kosmos-2 평균 1278ms(가장 빠르지만 정답 0건이라 무효), OWLv2 평균
  1952ms(가장 일정, 1916~2021ms), PG2 평균 2936ms(다중박스 시도 시 최대
  8270ms까지 튐)
- 로컬 리포트: 그라운딩 인코더 어블레이션 형태로 정리(속도차트+오프라인
  ablation 표까지 포함), 필요하면 파일로 전달 가능

**실패 케이스 유형 2종 확정 (심사위원1 의견5, 심사위원3 의견7 대응)** —
위험한 개별 수치(그라운딩% 등) 노출 없이 정성적으로만 정리:
1. **그라운딩 손실형** — OWL-v2가 세션 전 구간 미검출(has_bbox=False),
   정책이 fallback 신호로 주행하다 목표 도달 실패. 예: pos2_강우 세션
   20260917_123659
2. **정책 과잉대응형** — 그라운딩은 정상인데 정책이 한쪽으로 과커밋해
   목표가 화면 밖으로 밀려남. 예: pos3_강우 세션 20260917_193352
- 각 유형을 같은 path_type의 성공 세션과 짝지어 실제 actions 적분 궤적으로
  비교(절대거리 아닌 모양 비교용) — 실패는 보정 없이 직진하거나 전진
  없이 옆이동만 계속하는 패턴, 성공은 같은 초기 반응 후 정상 복귀/전환

**코드 버그 수정 2건**:
- `action_speed_scale`이 실제로 모터에 반영 안 되던 버그 — ①
  `vla_control_utils.py`의 `publish_and_move()`가 액션 크기(magnitude)를
  무시하고 고정 throttle만 쓰던 것, ② `InferenceResponse` Pydantic 모델에
  필드 선언은 됐는데 `/predict` 생성자 호출에서 빠져서 응답에서 잘려나가던
  것 — 둘 다 수정 완료, 이후 실측으로 pos3_강우 성공률이 33%→80%까지
  회복되는 것으로 효과 확인됨
- `stop_on_miss`(미검출 시 액션 강제 0) 신규 런타임 옵션 추가, 기본 off
- **미수정 채로 보류**: `end_reason`(세션이 모델 자동종료인지 수동종료인지
  구분하는 필드)이 코드상 존재하지 않는 키(`goal_near`)를 체크하는 버그로
  항상 "manual"만 기록되고 있음 — 확인은 했으나 실기 진행 중이라 지금은
  안 고치기로 함(향후 과제)

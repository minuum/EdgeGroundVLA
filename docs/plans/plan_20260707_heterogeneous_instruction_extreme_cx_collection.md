...
- [ ] 트랙C(오버슈트→재수렴) 시나리오 키 추가 — CH62 근거, 신규
      - 순환축 배치: 기존 D-pad 좌/우 순환 축(트랙A cx위치 / 접근경로 / 시나리오)과
        **같은 구역·같은 행**에 "오버슈트 방향"(과좌회전→우보정 / 과우회전→좌보정) 4번째
        순환 축으로 추가. D-pad 상/하로 4축(cx위치/접근경로/시나리오/오버슈트방향) 전환,
        좌/우로 각 축 내 값 순환 — 기존 3축과 동일한 상호작용 패턴 유지(녹화 중엔 변경 불가,
        L1/SEL 시작 시 선택된 값으로 태깅).
      - **화면 레이아웃 배치(2026-07-14 스크린샷 기준 최종 확정)**: 실제 대시보드는 3열
        그리드 — 1열(넓게)=실시간 카메라(CX 오버레이), 2열=조이스틱(DRAGONRISE) 조작
        안내 블록, 3열=트랙A 극단배치 진행률(CX축 막대그래프)+시나리오&진행률. **2열
        조이스틱 블록 바로 아래, 현재 빈 검은 박스로 남아있는 자리에 "📼 에피소드
        제어"(episode_name/▶시작/⏹정지&저장/🔄복귀) 블록과 그 아래 "📊 현재 에피소드
        타임라인"(기호 시퀀스+실측 Hz) 블록을 배치**(2열 내 세로 2번째 칸). 3열의 "트랙A
        극단배치 진행률"/"시나리오&진행률" 패널은 그대로 유지 — 여기에 트랙C용 4번째
        순환축(오버슈트 방향) 및 그 진행률 카운트(위치4×방향2×8회=64)도 같은 방식으로
        추가. ※ 이전 두 차례 정정(오버슈트축 위젯 오배치, 위치 불명확) 모두 무효화하고
        이 스크린샷 기준 버전이 최종.
- [x] 트랙A 극단cx 4위치×3경로(180ep) 물리 수집 완료 — 2026-07-16
- [ ] 트랙C(오버슈트→재수렴, 64ep) 물리 수집
- [ ] **트랙F(center 위치, 45ep, §1-2 참고) 물리 수집 — 2026-07-16 신규 요청.** cx_position
      선택지에 `center` 옵션 추가(기존 4위치와 같은 UI, 신규 위젯 불필요) 후 동일 3경로
      × 15회. exp73(트랙A 단독 학습 실험) 진행 중 V6에 center 데이터가 전혀 없어
      V5(시간축 이질적) 혼합 없이는 중앙 상황을 학습·검증 못 한다는 문제 확인됨 —
      V6를 메인으로 갈 경우 이 45ep이 없으면 V5 의존을 못 끊음.
- [ ] 재학습(150ep+트랙A/C 244ep+트랙F 45ep+basket free21=460ep, 또는 V6 완결 시
      V6 단독 225ep+트랙C 64ep=289ep로 대체 가능) + §5 평가지표(VSC/오버슈트회복률/

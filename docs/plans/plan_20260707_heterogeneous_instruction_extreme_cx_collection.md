...
- [ ] 트랙C(오버슈트→재수렴) 시나리오 키 추가 — CH62 근거, 신규
      ...
      - **화면 레이아웃 배치(2026-07-12 수정)**: 대시보드 상단 "🕹️ 조작(키보드 W/A/S/D/Q/E/Z/C/R/T)"
        블록과 **같은 행**에 이미 2:1:1 컬럼 비율 구조가 있음 — 1열(2fr)=키보드 조작 블록
        유지, **중간 열(가운데 1fr)에 트랙C "오버슈트 방향" 순환축 UI를 배치**(D-pad
        상/하/좌/우와 별개로, 현재 선택된 오버슈트 방향 값을 이 중간 칸에 크게 표시).
        3열(마지막 1fr)은 기존 조이스틱(DragonRise) 블록 그대로 유지.

#!/usr/bin/env python3
"""
exp65: Stage2 MLP 재학습 — base PG2 grounding cx 사용 (깨끗한 기준선)

exp60(exp59 LoRA cx)와 달리 base PG2 cx 입력. grounding 소스만 차이로 통제.
CL 평가에서 exp65(base) vs exp60(exp59) vs exp54(HSV) 비교.

Usage:
  # Step1: base PG2 annotation 생성 (먼저 실행)
  .venv/bin/python3 scripts/gen_base_pg2_annotation.py
...
def main():
    ...
    parser.add_argument("--ann", type=str, default=str(ANN_PG2), help="학습 annotation")
    parser.add_argument("--out", type=str, default=str(OUT_DIR), help="출력 디렉토리")
    args = parser.parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    random.seed(args.seed); np.random.seed(args.seed); torch.manual_seed(args.seed)

    ann_path = Path(args.ann); out_dir = Path(args.out); out_dir.mkdir(parents=True, exist_ok=True)
    if not ann_path.exists():
        print(f"ERROR: {ann_path} 없음. gen_base_pg2_annotation.py 먼저 실행하세요.")
        sys.exit(1)

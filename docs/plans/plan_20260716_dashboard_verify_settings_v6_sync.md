...
> 상태: **Phase 1 완료(트랙A 12종 라벨 추가, 커밋 `2640b19e`) +
> 후속 수정 완료(커밋 `3b289427`)**: `<select id="ep-path-type">`에
> 트랙A `<option>`이 누락돼 퀵라벨 버튼이 실제로는 동작하지 않던 버그
> 수정, `TRACKA_UNCOLLECTED` stale 데이터(strong_left 미수집 표시)
> 제거(180/180 전체 수집 완료 반영), 기본 선택값을 V6
> (`trackA_weak_left_left_curve`)로 변경. Phase 2/3/4는 여전히 미착수.

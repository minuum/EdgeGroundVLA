#!/usr/bin/env python3
"""
plan_20260622_text_prompt_hidden_state_direction.md — CH38-5("방향 텍스트를
줘도 생성 출력이 안 바뀐다")의 hidden-state판. 같은 V5 220장에 대해 prompt를
P0="detect gray basket" / P1="...on the left" / P2="...on the right"로
바꿔가며 hidden state를 추출하고, 세 가지를 probe로 비교한다:

  1. 이미지의 진짜 방향(파일명 라벨) vs P0 hidden state — CH39 재확인
  2. 이미지의 진짜 방향 vs P1/P2 hidden state — prompt가 틀려도 이미지 신호가 읽히는가
  3. prompt 종류(P0/P1/P2) 자체 vs hidden state — 같은 이미지인데 prompt만 바꿔도 갈리는가

새 데이터 수집 없음(기존 220 에피소드), 새 학습 없음(frozen probe만).

산출: docs/v5/attention_analysis/v5_direction_text_prompt_probe.json

Usage:
  .venv/bin/python3 scripts/eval/probe_v5_direction_text_prompt.py
"""
import json
import re
...
PROMPTS = {
    ...
    "P1_left": "detect gray basket on the left",
    "P2_right": "detect gray basket on the right",
}
...
def main():
    ...
    out_path = OUT_DIR / "v5_direction_text_prompt_probe.json"

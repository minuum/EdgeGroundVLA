#!/usr/bin/env python3
"""
Window ablation CL 평가 — exp71 Transformer WINDOW=4/6/8/12/16
각 체크포인트의 SR + FPE 측정.

Usage:
  .venv/bin/python3 scripts/eval_window_cl.py
"""
...
CLIP_PROJ = 256; CLIP_VIS = 1024; NUM_CLASSES = 8
WINDOWS = [4, 6, 8, 12, 16]
...
def eval_one_episode(ann_ep, head, window, proc, vm, proj, device, dt=DT_DEFAULT, success_fpe=0.5):
    ...
    return compute_metrics(expert_traj, pred_traj, success_fpe)
...
def main():
    ...
    for w in WINDOWS:
        # WINDOW=8은 seed42 체크포인트 사용 (기존 runs/exp71/)
        if w == 8:
            pt = ROOT / "runs/v5_nav/mlp/exp71/action_transformer.pt"
        else:
            pt = ROOT / f"runs/v5_nav/mlp/exp71_window{w}/action_transformer.pt"

        if not pt.exists():
            print(f"[SKIP] WINDOW={w}: {pt} 없음")
            results[f"w{w}"] = None
            continue

        ckpt = torch.load(str(pt), map_location=device, weights_only=False)
        head = _TransHead(window=w).to(device)
        ...
        for ann_ep in test_eps:
            try:
                m = eval_one_episode(ann_ep, head, w, proc, vm, proj, device)
        ...
        results[f"w{w}"] = {"window": w, "val_acc": val_acc,
                             "cl_sr": sr_mean, "cl_fpe": fpe_mean, "n_eps": len(srs)}
        print(f"  WINDOW={w:2d}: val_acc={val_acc:.1f}%  CL_SR={sr_mean:.1f}%  FPE={fpe_mean:.3f}m  ({len(srs)} eps)")

    # 결과 저장
    out = LOG_DIR / "cl_window_summary.json"
    with open(out, "w") as f:
        json.dump(results, f, indent=2)
    print(f"\n[DONE] → {out}")

    print("\n=== Window Ablation 요약 ===")
    print(f"{'WINDOW':>8} {'val_acc':>9} {'CL_SR':>8} {'FPE':>8}")
    print("-" * 40)
    for k, v in results.items():
        if v:
            print(f"  {v['window']:>6}   {v['val_acc']:>7.1f}%  {v['cl_sr']:>6.1f}%  {v['cl_fpe']:>7.3f}m")

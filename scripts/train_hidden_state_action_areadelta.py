...
STAGE2_DIR   = ROOT / "runs" / "v5_nav" / "mlp" / "exp_hidden_state" / "stage2_v2_areadelta"
...
PROJ_DIM    = 256
HIDDEN_DIM  = 2304
D_IN_BASE   = WINDOW * 5 + PROJ_DIM   # 296 (area_delta 채널 추가)
...
def bbox_feat(frames, t, window=WINDOW):
    ...
    회전 직전 프레임 자체의 area는 거의 무분산이지만(CH46-5), 윈도우가 포함하는
    직전 FORWARD 구간의 area 상승 추세는 살아있을 수 있어 그 변화율을 명시적으로 노출."""
    arr = []
    for k in range(window):
        ...
        prev_area = frames[prev_idx].get("area", area)

...
OUT_DIR          = ROOT / "runs/v5_nav/grounding/exp64_visionenc"
...
def main():
    ...
    # [exp64] vision 인코더(SigLIP) 상위 8레이어(19~26)만 LoRA — "객체 인식 인코더만 학습"
    vis_targets = [f"vision_tower.vision_model.encoder.layers.{L}.self_attn.{m}_proj"
                   for L in range(19, 27) for m in ("q", "k", "v")]
    lora_cfg = LoraConfig(
        r=args.lora_r,
    ...
    # [exp64] 전 모델에서 LoRA만 trainable (vision-only 타겟이므로 = vision 인코더 LoRA)
    n_tr = 0
    for name, p in model.named_parameters():
        p.requires_grad = ("lora_" in name)
        if p.requires_grad:
            p.data = p.data.float(); n_tr += 1
    print(f"[exp64] trainable(lora) tensors: {n_tr}", flush=True)

...
STAGE2_DIR   = ROOT / "runs" / "v5_nav" / "mlp" / "exp_hidden_state" / "stage2_v2_lstm_areadelta"
...
def bbox_one(fr, prev_fr=None):
    """plan_20260623_area_delta_proximity_feature.md — area_delta(5번째 채널) 추가."""
    cx = fr.get("cx", 0.5); cy = fr.get("cy", 0.5)
    area = fr.get("area", 0.05); has = float(fr.get("has_bbox", False))
    prev_area = prev_fr.get("area", area) if prev_fr is not None else area
    delta = area - prev_area
    return np.array([cx, cy, area, has, delta], dtype=np.float32)
...
def build_seq(frames, feats, t, hidden_cache, ep_path, mode, window=WINDOW):
    ...
    for k in range(window):
        ...
        fr = frames[ti]
        prev_fr = frames[max(0, ti - 1)]
        img_seq.append(feats[ti])
        bbox_seq.append(torch.tensor(bbox_one(fr, prev_fr)))
        if mode != "none":
